...
export class Migration20250605082948 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table if not exists "product_media" ("id" text not null, "type" text check ("type" in ('image', 'video')) not null, "url" text not null, "product_id" text not null, "title" text null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "product_media_pkey" primary key ("id"));`);
