...
## Задача: Восстановление работоспособности сервера (КРИТИЧЕСКАЯ)
- **Статус**: ЗАБЛОКИРОВАНО (Требует немедленного вмешательства)
- **Описание**: Сервер, на котором развернут проект Ugodo, не загружает операционную систему (ошибка "No init found", вход в `initramfs`). Это полностью блокирует всю работу над проектом.
- **Шаги выполнения**:
  - [ ] Получить доступ к консоли сервера (через IPMI или физически).
  - [ ] Попытаться диагностировать точную причину сбоя (логи, состояние дисков).
  - [ ] **Вариант 1: Попытка восстановления существующей ОС**
    - [ ] Загрузиться с Live CD/USB.
    - [ ] Проверить и восстановить файловую систему (например, с помощью `fsck`).
    - [ ] Проверить и восстановить загрузчик (например, GRUB).
    - [ ] Провести диагностику оборудования (особенно жестких дисков).
  - [ ] **Вариант 2: Восстановление ОС из резервной копии**
    - [ ] Если существует актуальная резервная копия ОС, восстановить систему из нее.
  - [ ] **Вариант 3: Переустановка Операционной Системы (крайний случай)**
    - [ ] Подготовить установочный носитель.
    - [ ] Установить ОС.
    - [ ] Восстановить конфигурации сервера (сеть, пользователи и т.д.).
    - [ ] **Восстановить проект Ugodo:**
        - [ ] Установить необходимое ПО (PostgreSQL, Minio, Nginx, Node.js, Yarn, Docker и т.д.).
        - [ ] Восстановить базу данных PostgreSQL из резервной копии (`pg_restore`).
        - [ ] Восстановить данные Minio из резервной копии.
        - [ ] Восстановить файлы проекта из Git-репозитория.
        - [ ] Настроить переменные окружения (`.env`).
        - [ ] Настроить Nginx.
        - [ ] Запустить все сервисы проекта.
  - [ ] Тщательно протестировать работоспособность сервера и всех сервисов проекта Ugodo.
- **Зависимости**: Доступ к серверу, возможно, загрузочные носители, наличие резервных копий ОС, БД и данных Minio.

## Задача: Внедрение Tiered Pricing (Оптовые скидки по количеству)
- **Статус**: Готово к тестированию
- **Описание**: Реализовать функционал скидок, зависящих от количества заказываемого товара. Цены должны автоматически изменяться в корзине и на странице товара при изменении количества. **Уточнение**: Расчет tiered pricing происходит на бэкенде Medusa. Фронтенд отображает цены, полученные от API.
- **Шаги выполнения**:
  - [x] Анализ API Medusa и структуры проекта
  - [x] Создание и обновление документации (changelog.md, tasktracker.md, project.md)
  - [x] Уточнение, что фронтенд не рассчитывает tiered-цены, а отображает данные от API.
  - [x] Проверка и коррекция `getProductPrice` для отображения базовой цены.
  - [x] Проверка компонентов страницы товара: отображение базовой цены, добавлено инфо-сообщение.
  - [x] Проверка компонентов корзины: отображение цен из данных корзины (unit_price, total) - изменений не потребовалось.
  - [ ] Тестирование отображения цен (после настройки tiered-цен в бэкенде/админке Medusa):
    - [ ] Настройка tiered prices для тестовых товаров в админ-панели Medusa.
    - [ ] Проверка отображения базовой цены и инфо-сообщения на странице товара.
    - [ ] Проверка автоматического изменения unit_price и total в корзине при изменении количества товара.
- **Зависимости**: Корректная настройка tiered-цен в бэкенде Medusa (через админку или API).