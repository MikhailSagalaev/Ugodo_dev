...
## Задача: Разделить и починить регистрацию и авторизацию по SMS (OTP)
- **Статус**: Завершена
- **Описание**: Регистрация и авторизация по SMS разделены на отдельные страницы и независимые флоу. Исправлен баг: теперь после регистрации пользователь создаётся и автоматически входит.
- **Шаги выполнения**:
  - [x] Разделить компоненты LoginSms и RegisterSms
  - [x] Создать отдельные страницы /account/login-sms и /account/register-sms
  - [x] Исправить роутинг и layout
  - [x] Починить регистрацию: после OTP создаётся пользователь через /store/customers
  - [x] Добавить автоматический вход после регистрации
- **Зависимости**: Требует актуальной версии PerseidesJS Auth-OTP и Medusa backend
...
## Задача: Рефакторинг SMS-авторизации/регистрации (PerseidesJS)
- **Статус**: Завершена
- **Описание**: Улучшить UX и обработку ошибок для флоу регистрации и входа по SMS (OTP) через PerseidesJS/Medusa. Покрыть все сценарии, добавить логирование, улучшить тексты сообщений.
- **Шаги выполнения**:
  - [x] Проанализировать текущий флоу и типовые ошибки PerseidesJS
  - [x] Добавить функцию парсинга ошибок PerseidesJS
  - [x] Улучшить UX copy для всех сценариев
  - [x] Добавить подробное логирование ошибок
  - [x] Покрыть сценарии: регистрация, логин, несуществующий пользователь, ошибки сети и кода
  - [x] Провести code review и рефакторинг компонента
- **Зависимости**: Флоу PerseidesJS, компонент sms-auth/page.tsx