...
## [2024-07-31] - Разделение регистрации и авторизации по SMS на отдельные флоу (frontend)
### Добавлено
- Созданы отдельные компоненты LoginSms и RegisterSms для login/register по SMS (OTP)
- Созданы страницы /account/login-sms и /account/register-sms
- Добавлены actions-login.ts и actions-register.ts для разделения логики

### Изменено
- Компоненты Login и Register теперь ведут только на соответствующие SMS-страницы
- Email/пароль полностью скрыт из интерфейса
- Навигация и UX обновлены для явного выбора login/register

### Исправлено
- Нет

## [2024-07-31] - Исправлен роутинг и UX страниц аккаунта
### Добавлено
- Создана страница /account с выбором между входом и регистрацией по SMS

### Изменено
- Обновлен layout.tsx для account: теперь универсальный, children рендерится всегда
- Исправлен роутинг: /account/login-sms и /account/register-sms работают напрямую

### Исправлено
- Устранена проблема Page not found для /account/login-sms и /account/register-sms
- /account больше не пустая, корректно отображает выбор для пользователя

## [2024-07-31] - Исправлен flow регистрации по SMS (OTP)
### Добавлено
- Создание пользователя через /store/customers после успешной верификации OTP
- Автоматический вход после регистрации через /auth/customer/otp/authenticate

### Изменено
- Логика компонента RegisterSmsPage: теперь регистрация полностью соответствует PerseidesJS и Medusa

### Исправлено
- Проблема, при которой пользователь не создавался после регистрации по SMS