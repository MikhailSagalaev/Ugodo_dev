...
const RegisterSms = () => {
  ...
  const [flow, setFlow] = useState<'register' | 'login' | ''>('')
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let response
      try {
        response = await fetch(`${API_URL}/auth/customer/otp/pre-register`, {
          method: 'POST',
        ...
        console.log('[RegisterSms] Real API unavailable, using test API')
        setUseTestApi(true)
        const result = await testSmsApi.preRegister(phone)
        if (result.success) {
          setFlow('register')
          setSuccess('SMS с кодом отправлен! (Тестовый режим)')
          setTimeout(() => setStep('otp'), 1000)
        } else {
          if (result.message?.includes('Actor already exists')) {
            setFlow('login')
            setSuccess('Пользователь уже существует. Перенаправляем на вход...')
            setTimeout(() => router.push('/account/login-sms'), 2000)
          } else {
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (useTestApi) {
        // Используем тестовый API - полный процесс регистрации
        const registerResult = await testSmsApi.register(phone, otp)
        if (!registerResult.success) {
          setError(registerResult.message || 'Ошибка получения токена регистрации')
          return
        }

        const createResult = await testSmsApi.createCustomer(registerResult.registration_token!, phone)
        if (!createResult.success) {
          setError(createResult.message || 'Ошибка создания пользователя')
          return
        }

        const authResult = await testSmsApi.authenticate(phone, otp)
        if (authResult.success) {
          const authToken = authResult.token || authResult.access_token || 'test_token'
          await setMedusaAuthToken(authToken)
          setSuccess('Регистрация завершена! Добро пожаловать! (Тестовый режим)')
          setTimeout(() => {
            redirectToAccount()
          }, 1500)
        } else {
          setSuccess('Аккаунт создан! Переходим к входу...')
          setTimeout(() => router.push('/account/login-sms'), 1500)
        }
      } else {
        // Используем реальный API с поддержкой тестового режима
        // Шаг 1: Получаем registration_token
        const registerEndpoint = TEST_MODE ? 
          `${API_URL}/auth/customer/otp/register-test` : 
          `${API_URL}/auth/customer/otp/register`
        ...
        if (registerRes.status !== 200) {
          ...
          setError(data.message || 'Ошибка получения токена регистрации')
          return
        }
        ...
        if (!registrationToken) {
          setError('Не получен токен регистрации')
          return
        }

        // Шаг 2: Создаем customer используя registration_token  
        // Согласно документации, передаем phone в качестве основного идентификатора
        const createCustomerRes = await fetch(`${API_URL}/store/customers`, {
          ...
          body: JSON.stringify({ 
            ...
            last_name: 'SMS',
            // Согласно конфигурации, используем phone как идентификатор
            email: `${phone.replace(/\D/g, '')}@temp.local` // Временный email для совместимости
          })
        })
        ...
        console.log('[RegisterSms] Customer created:', customerData)

        // Обновляем auth_identity чтобы связать с созданным customer
        if (registerData.auth_identity?.id && customerData.customer?.id) {
          try {
            await fetch(`${API_URL}/auth/customer/otp/update-auth-identity`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                auth_identity_id: registerData.auth_identity.id,
                customer_id: customerData.customer.id
              })
            })
            console.log('[RegisterSms] Auth identity updated successfully')
          } catch (error) {
            console.warn('[RegisterSms] Failed to update auth identity:', error)
          }
        }

        // Шаг 3: Автоматический вход через authenticate  
        // Согласно документации, тот же OTP можно переиспользовать в течение 60 секунд
        const authEndpoint = TEST_MODE ? 
          `${API_URL}/auth/customer/otp/authenticate-test` : 
          `${API_URL}/auth/customer/otp/authenticate`
        ...
        if (authRes.status === 200) {
          ...
          if (authToken) {
            await setMedusaAuthToken(authToken)
            setSuccess('Регистрация завершена! Добро пожаловать!')
            setTimeout(() => {
              redirectToAccount()
            }, 1500)
          } else {
      ...
      <div className="w-full">
        ...
        <div className="text-center mb-8">
          ...
          <Text className="text-gray-600">
            Введите номер телефона для создания нового аккаунта
          </Text>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="pl-10 block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500"
                required
                autoComplete="tel"
              />
            </div>
        ...
        <div className="mt-6 text-center">
          <Text className="text-sm text-gray-600">
            Уже есть аккаунт?{' '}
            <button
              onClick={() => router.push('/account/login-sms')}
  ...
  return (
    <div className="w-full">
      ...
      <div className="text-center mb-8">
        ...
        <Heading level="h1" className="text-2xl font-bold text-gray-900 mb-2">
          {flow === 'register' ? 'Введите код' : 'Пользователь уже существует'}
        </Heading>
        <Text className="text-gray-600">
          {flow === 'register' 
      ...
      {/* Form - только если это регистрация */}
      {flow === 'register' && (
