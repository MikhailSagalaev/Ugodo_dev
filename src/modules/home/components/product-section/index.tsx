...
export default function ProductSection({ 
  ...
  return (
    <div className={`w-full py-12 ${variant === "colored" ? "bg-gradient-to-r from-violet-50 to-pink-50" : ""}`}>
      <div className="content-container">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <Text className="text-2xl font-semibold">{title}</Text>
            {variant === "colored" && (
              <p className="text-gray-600 mt-1 text-sm">Специальные предложения и акции</p>
            )}
          </div>
          
          {link && (
            <InteractiveLink href={link.href} className="mt-2 md:mt-0 text-violet-600 hover:text-violet-700 flex items-center gap-1 group">
              <span>{link.text}</span>
              <svg 
